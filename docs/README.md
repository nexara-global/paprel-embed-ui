...
Use `@paprel/embed-core` for shared auth and transport, then install complete domain packages such as `@paprel/embed-accounting`. Each domain owns its typed resources and UI. `@paprel/embed-ui` supplies the inherited theme contract without requiring a separate consumer import.
...
| `packages/ui` | `@paprel/embed-ui` |
| `packages/accounting` | `@paprel/embed-accounting` |
| `packages/reports` | `@paprel/embed-reports` — all financial report components |
